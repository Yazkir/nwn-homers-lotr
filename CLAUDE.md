...
**`docs/` is volatile — it is fully regenerated by `nwn-manager wiki` and must
not be hand-edited.** Any manual additions placed directly in `docs/` will be
wiped on the next wiki rebuild. To add or update wiki content permanently:

- **Manual/narrative pages** (player guides, quest compendiums, etc.) → add or
